...
# Maximum security flags for toolchain build (compatible with 2012 era compilers)
SECURE_CFLAGS="-O2 -fstack-protector -fPIC -D_FORTIFY_SOURCE=2 -Wformat -Wformat-security"
SECURE_CXXFLAGS="${SECURE_CFLAGS}"
SECURE_LDFLAGS="-Wl,-z,relro -Wl,-z,now"
...
setup_environment() {
    ...
    umask 0077
    mkdir -p "${BUILD_DIR}"/{llvm,binutils,gcc-stage1,gcc-stage2,newlib}
    mkdir -p "${INSTALL_DIR}"/{bin,lib,include,arm-none-eabi}
    mkdir -p "${CONSOLE_DIR}"
...
extract_archives() {
    ...
    # Extract GCC
    if [[ -f "${ARCHIVE_DIR}/gcc-4.7.2.tar.bz2" ]]; then
        log_info "Extracting GCC..."
        tar --no-same-owner --no-same-permissions -xjf "${ARCHIVE_DIR}/gcc-4.7.2.tar.bz2" || \
        tar -xjf "${ARCHIVE_DIR}/gcc-4.7.2.tar.bz2"
    fi
    
    # Extract newlib (note: actual source is nested)
    if [[ -f "${ARCHIVE_DIR}/newlib-xtensa-newlib-2_0_0.tar.gz" ]]; then
        log_info "Extracting newlib..."
        tar --no-same-owner --no-same-permissions -xzf "${ARCHIVE_DIR}/newlib-xtensa-newlib-2_0_0.tar.gz" || \
        tar -xzf "${ARCHIVE_DIR}/newlib-xtensa-newlib-2_0_0.tar.gz"
        
        # Find the actual newlib source directory
        NEWLIB_DIR=$(find . -type d -name "newlib" -path "*/src/newlib" | head -1)
        if [[ -n "$NEWLIB_DIR" ]]; then
            log_success "Found newlib source at: $NEWLIB_DIR"
            # Create symlink for easier access
            ln -sf "$(dirname $(dirname $NEWLIB_DIR))" newlib-src
        else
            log_warning "Could not locate newlib source directory"
        fi
    fi
...
build_binutils() {
    ...
    "${WORK_DIR}/binutils-2.23.1/configure" \
        ...
        --prefix="${INSTALL_DIR}" \
        --with-sysroot="${INSTALL_DIR}/arm-none-eabi" \
        --disable-nls \
        --disable-werror \
        --enable-gold \
...
build_gcc_stage1() {
    log_info "Building GCC stage 1 (compiler only)..."
    
    mkdir -p "${BUILD_DIR}/gcc-stage1"
    cd "${BUILD_DIR}/gcc-stage1"
    
    if [[ ! -f "${WORK_DIR}/gcc-4.7.2/configure" ]]; then
        log_error "GCC source not found at ${WORK_DIR}/gcc-4.7.2"
        return 1
    fi
    
    # GCC 4.7.2 requires these specific versions (from 2012 era)
    # GMP 4.3.2+, MPFR 2.4.2+, MPC 0.8.1+
    cd "${WORK_DIR}/gcc-4.7.2"
    
    # Download prerequisites manually for GCC 4.7.2
    if [[ ! -d gmp-4.3.2 && ! -d gmp ]]; then
        log_info "Downloading GMP 4.3.2..."
        wget -q ftp://gcc.gnu.org/pub/gcc/infrastructure/gmp-4.3.2.tar.bz2 || \
        wget -q https://ftp.gnu.org/gnu/gmp/gmp-4.3.2.tar.bz2 || \
        wget -q https://gmplib.org/download/gmp/archive/gmp-4.3.2.tar.bz2 || true
    ...
    if [[ ! -d mpfr-2.4.2 && ! -d mpfr ]]; then
        log_info "Downloading MPFR 2.4.2..."
        wget -q ftp://gcc.gnu.org/pub/gcc/infrastructure/mpfr-2.4.2.tar.bz2 || \
        wget -q https://www.mpfr.org/mpfr-2.4.2/mpfr-2.4.2.tar.bz2 || \
        wget -q https://ftp.gnu.org/gnu/mpfr/mpfr-2.4.2.tar.bz2 || true
    ...
    if [[ ! -d mpc-0.8.1 && ! -d mpc ]]; then
        log_info "Downloading MPC 0.8.1..."
        wget -q ftp://gcc.gnu.org/pub/gcc/infrastructure/mpc-0.8.1.tar.gz || \
        wget -q https://ftp.gnu.org/gnu/mpc/mpc-0.8.1.tar.gz || \
        wget -q http://www.multiprecision.org/downloads/mpc-0.8.1.tar.gz || true
    ...
    # Configure without system zlib for GCC 4.7.2
    "${WORK_DIR}/gcc-4.7.2/configure" \
        --target=arm-none-eabi \
        --prefix="${INSTALL_DIR}" \
        --with-sysroot="${INSTALL_DIR}/arm-none-eabi" \
        --with-newlib \
...
build_newlib() {
    log_info "Building newlib..."
    
    mkdir -p "${BUILD_DIR}/newlib"
    cd "${BUILD_DIR}/newlib"
    
    # Find the newlib source (it's nested)
    NEWLIB_SRC="${WORK_DIR}/newlib-src"
    if [[ ! -d "$NEWLIB_SRC" ]]; then
        NEWLIB_SRC=$(find "${WORK_DIR}" -type d -name "src" -path "*newlib*" | head -1)
        if [[ ! -d "$NEWLIB_SRC" ]]; then
            log_error "Cannot find newlib source directory"
            return 1
        fi
    fi
    
    log_info "Using newlib source at: $NEWLIB_SRC"
    
    # Configure newlib for bare metal
    ...
    PATH="${INSTALL_DIR}/bin:${PATH}" make -j$(nproc) MAKEINFO=true
    PATH="${INSTALL_DIR}/bin:${PATH}" make install MAKEINFO=true
    
    cd - > /dev/null
    log_success "Newlib built"
}

build_gcc_stage2() {
    log_info "Building GCC stage 2 (with newlib)..."
...
build_clang() {
    ...
    # Configure for ARM bare metal with GCC backend support
    PATH="${INSTALL_DIR}/bin:${PATH}" \
    "${WORK_DIR}/llvm-3.2.src/configure" \
        ...
        --enable-assertions \
        --enable-pic \
        --with-gcc-toolchain="${INSTALL_DIR}" \
        --with-binutils-include="${WORK_DIR}/binutils-2.23.1/include" \
        CFLAGS="${SECURE_CFLAGS}" \
        CXXFLAGS="${SECURE_CXXFLAGS}" \
        LDFLAGS="${SECURE_LDFLAGS}"
    
    PATH="${INSTALL_DIR}/bin:${PATH}" make -j$(nproc) ENABLE_OPTIMIZED=1
    PATH="${INSTALL_DIR}/bin:${PATH}" make install
...
build_console() {
    ...
    # First, check if kernel.c exists in current directory or parent
    if [[ -f "${PWD}/../../kernel.c" ]]; then
        cp "${PWD}/../../kernel.c" universal_pi_console.c
        log_success "Found kernel.c in parent directory"
    elif [[ -f "${PWD}/../kernel.c" ]]; then
        cp "${PWD}/../kernel.c" universal_pi_console.c
        log_success "Found kernel.c in working directory"
    else
        log_error "Console source (kernel.c) not found!"
        log_info "Please copy your console source code to kernel.c in the current directory"
        return 1
    fi
    ...
    # Common security flags for bare metal (with libgcc and newlib)
    BARE_METAL_FLAGS=(
        -ffreestanding
        -nostartfiles
        -fno-builtin
        ...
        -O2
        -g
        -I"${INSTALL_DIR}/arm-none-eabi/include"
        -L"${INSTALL_DIR}/arm-none-eabi/lib"
        -L"${INSTALL_DIR}/lib/gcc/arm-none-eabi/4.7.2"
    )
...
main() {
    ...
    # Check for kernel.c first
    if [[ ! -f "kernel.c" ]]; then
        log_error "kernel.c not found in current directory!"
        log_info "Please copy your Universal Pi Console source code to kernel.c"
        exit 1
    fi
    ...
    build_gcc_stage1 || exit 1
    build_newlib || exit 1
    build_gcc_stage2 || exit 1
    build_clang || exit 1
