...
def generate_gdb_script(functions, globals_vars, output_file, options):
    ...
    with open(output_file, 'w') as f:
        ...
        f.write("        actual_str = str(actual).split()[0]  # Get just the address part\n")
        f.write("        # Normalize addresses for comparison\n")
        f.write("        if '0x' in actual_str:\n")
        f.write("            actual_addr = actual_str\n")
        f.write("        else:\n")
        f.write("            actual_addr = hex(int(actual_str))\n")
        f.write("        # Compare normalized addresses\n")
        f.write("        if actual_addr.lower() != expected_addr.lower():\n")
        f.write("            # Check if it's just a formatting difference\n")
        f.write("            try:\n")
        f.write("                if int(actual_addr, 16) == int(expected_addr, 16):\n")
        f.write("                    return True  # Same address, different format\n")
        f.write("            except:\n")
        f.write("                pass\n")
        ...
        f.write("            actual_str = str(actual).split()[0]\n")
        f.write("            if '0x' in actual_str:\n")
        f.write("                actual_addr = actual_str\n")
        f.write("            else:\n")
        f.write("                actual_addr = hex(int(actual_str))\n")
        ...
        f.write("    # Python 3 compatibility - no 'long' type\n")
        f.write("    try:\n")
        f.write("        old_int = int(old_val)\n")
        f.write("        new_int = int(new_val)\n")
        f.write("        if new_int == 0 and old_int != 0:\n")
        f.write("            print('    *** ALERT: Value zeroed out!')\n")
        f.write("        elif abs(new_int - old_int) > 0x10000:\n")
        f.write("            print('    *** ALERT: Large value jump detected!')\n")
        f.write("    except (ValueError, TypeError):\n")
        f.write("        pass  # Values aren't integers\n")
        f.write("    \n")
        f.write("    print('='*60 + '\\n')\n\n")
